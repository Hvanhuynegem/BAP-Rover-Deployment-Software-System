...
volatile unsigned char TXData = 150;
...
#endif
{
  switch(__even_in_range(UCA1IV, 18))
  {
    ...
    case 0x04: // Vector 4: UCTXIFG
      // The UCTXIFG interrupt flag is set by the transmitter to indicate that UCAxTXBUF is ready to accept another character.
      P1OUT |= BIT0;            // Turn on LED when TX buffer is empty
      __delay_cycles(20000);   // Delay for 100000 cycles
      UCA1TXBUF = TXData;       // Load data onto buffer
      P1OUT &= ~BIT0;           // Turn off LED after loading data
      __delay_cycles(10000);   // Delay for 100000 cycles
      break;
